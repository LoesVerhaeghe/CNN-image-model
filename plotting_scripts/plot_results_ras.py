...
output_path='output/settling/TSSras/output_extrainputs_TSSras_error_train3'
...
df=pd.read_excel('data/settler_data/filtered_data/filtered_TSS_data.xlsx',sheet_name='TSS_RAS', index_col=0)

y_predicted = np.array(y_predicted).ravel() +df['WeeklyMechModelOutput'].values
y_true = np.array(y_true).ravel() +df['WeeklyMechModelOutput'].values
...
# plt.plot(df['WeeklyMechModelOutput'], '.-', label='Mech model output', color='green')
