...
df=pd.read_csv('data/pileaute/beluchting/Qair_total_interpolated.csv', index_col=0)
...
for fold_im_features in all_img_features:      
    ...
    for folder in all_image_folders:
        ...
        if images_in_folder_count > 0:
            target_per_date[folder]=df['Qair_total'].loc[folder].item() # Save label per date
    ...
    train_indices_folders=np.arange(0, 60)
    val_indices_folders = np.arange(60, 80) 
    ...
    #_, _ = train_model(model_temp, train_loader, val_loader, optimizer, None, criterion, epochs=700)
    ...
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 80)
    ...
    plt.plot(df['Qair_total'][all_dates][seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.figure(figsize=(14, 3), dpi=200)
plt.rcParams.update({'font.size': 12})    
plt.plot(df['Qair_total'][all_dates].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.legend()
plt.savefig('/home/loesv/all_results/pileaute/Qair/CNN_LSTM.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/Qair/CNN_LSTM.pdf',  bbox_inches='tight', dpi=250)
plt.show()
...
y_true_train = df['Qair_total'][all_dates].iloc[seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test = df['Qair_total'][all_dates].iloc[seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):]
...
# 1:1 line
lims = [
    min(df['Qair_total'][all_dates].iloc[seq_len-1:].min(), predictions.min()),
        max(df['Qair_total'][all_dates].iloc[seq_len-1:].max(), predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/pileaute/Qair/CNN_LSTM_scatterplot.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/Qair/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
