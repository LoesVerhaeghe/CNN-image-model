...
mpl.rcParams["pdf.fonttype"] = 42   # keep text as text (TrueType)
...
output_path='output/pileaute/Qairres'
images_base_folder = 'data/pileaute/beluchting/microscopic_images_pileaute'
df=pd.read_csv('data/pileaute/beluchting/Qair_residuals_total_interpolated.csv', index_col=0)
...
for fold_im_features in all_img_features:      
    ...
    for folder in all_image_folders:
        ...
        if images_in_folder_count > 0:
            target_per_date[folder]=df['airflow_total_residual'].loc[folder].item() # Save label per date
    ...
    train_indices_folders=np.arange(20, 74)
    val_indices_folders = np.arange(0, 20) 
    ...
    optimizer = optim.Adam(model_temp.parameters(), lr=1e-3)
    criterion = nn.MSELoss()

    #first do this to estimate how many epochs needed
    trained_model_temp, _ = train_model(model_temp, train_loader, val_loader, optimizer, None, criterion, epochs=750)
    ...
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 74)
    ...
    #trained_model, best_val_loss=train_model(model, train_val_loader, None, optimizer, None, criterion, epochs=2)

    
    ### testing
    full_loader = DataLoader(dataset, batch_size=1, shuffle=False)
    all_preds=[]
    trained_model_temp.eval()
    with torch.no_grad():
        for X, y in full_loader:
            X, y = X.to(device), y.to(device).float()
            preds = trained_model_temp(X)
    ...
    plt.plot(df['airflow_total_residual'].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.figure(figsize=(14, 3), dpi=200)
plt.rcParams.update({'font.size': 12})    
plt.plot(df['airflow_total_residual'][all_dates][seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.xlabel("Time")
plt.ylabel("Qair residuals (L/min)")
plt.legend()
plt.savefig('/home/loesv/all_results/pileaute/Qair_residuals/CNN_LSTM.png',  bbox_inches='tight', dpi=250)
#plt.savefig('/home/loesv/all_results/pileaute/Qair_residuals/CNN_LSTM.pdf',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/Qair_residuals/CNN2.pdf', format="pdf", bbox_inches="tight", transparent=True, dpi=300)
plt.show()
...
y_true_train = df['airflow_total_residual'][all_dates][seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test = df['airflow_total_residual'][all_dates][seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):]
...
# 1:1 line
lims = [
    min(df['airflow_total_residual'][all_dates][seq_len-1:].min(), predictions.min()),
        max(df['airflow_total_residual'][all_dates][seq_len-1:].max(), predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/pileaute/Qair_residuals/CNN_LSTM_scatterplot.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/Qair_residuals/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
