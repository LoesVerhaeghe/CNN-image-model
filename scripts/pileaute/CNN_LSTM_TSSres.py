...
output_path='output/pileaute/TSSres/TSSres'
images_base_folder = 'data/pileaute/settler_data/filtered_data/images_pileaute_settling_filtered'
df_TSS=pd.read_csv('data/pileaute/settler_data/filtered_data/TSS_eff_error.csv', index_col=0)
...
for fold_im_features in all_img_features:      
    ...
    val_indices_folders = np.arange(45, 55) 
    ...
    scheduler =  torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, factor=0.85)
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 55)
    ...
    trained_model, best_val_loss=train_model(model, train_val_loader, None, optimizer, None, criterion, epochs=200)
    ...
    if best_val_loss < 300: #only use this one for final predictions is validation loss is ok
    ...
    # plt.plot(df['LabMeasurement'].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
...
# --- Construct Model preds ---
TSS_predictions = pd.Series(
    avg_preds+df['MechModelOutput'].values[seq_len-1:],
    index=all_dates[seq_len-1:]
)
...
plt.figure(figsize=(14, 3), dpi=200)
plt.rcParams.update({'font.size': 12})    
plt.plot(df['LabMeasurement'][all_dates].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
plt.plot(TSS_predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)], '.-', label='Model predictions (train)', color='orange')
plt.plot(TSS_predictions.iloc[len(train_indices_folders)+len(val_indices_folders):], '.-', label='Model predictions (test)', color='red')
plt.xlabel("Time")
plt.ylabel("TSS effluent (mg/L)")
plt.legend()
plt.savefig('/home/loesv/all_results/pileaute/TSS_eff_residuals/CNN_LSTM.png', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/TSS_eff_residuals/CNN_LSTM.pdf', dpi=250)
plt.show()
...
y_true_train = df['LabMeasurement'][all_dates].iloc[seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = TSS_predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test = df['LabMeasurement'][all_dates].iloc[seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):]
y_pred_test = TSS_predictions.iloc[len(train_indices_folders)+len(val_indices_folders):]
...
# 1:1 line
lims = [
    min(df['LabMeasurement'][all_dates].iloc[seq_len-1:].min(), TSS_predictions.min()),
        max(df['LabMeasurement'][all_dates].iloc[seq_len-1:].max(), TSS_predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/pileaute/TSS_eff_residuals/CNN_LSTM_scatterplot.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/TSS_eff_residuals/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
