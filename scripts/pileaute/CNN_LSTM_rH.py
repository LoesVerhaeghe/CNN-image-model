...
df=pd.read_csv('data/pileaute/settler_data/all/rh_interpolated_all.csv', index_col=0)
...
for fold_im_features in all_img_features:      
    ...
    for folder in all_image_folders:
        ...
        if images_in_folder_count > 0:
            target_per_date[folder]=df['rH'].loc[folder].item() # Save label per date
    ...
    val_indices_folders = np.arange(50, 62) 
    test_indices_folders=np.arange(62, 111)
    ...
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 62)
    ...
    plt.plot(df['rH'][all_dates].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.ylabel("rH (L/g)")
plt.legend()
plt.savefig('/home/loesv/all_results/pileaute/rH/CNN_LSTM.png', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/rH/CNN_LSTM.pdf', dpi=250)
plt.show()
...
y_true_train = df['rH'][all_dates].iloc[seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test = df['rH'][all_dates].iloc[seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):]
...
# 1:1 line
lims = [
    min(df['rH'][all_dates].iloc[seq_len-1:].min(), predictions.min()),
        max(df['rH'][all_dates].iloc[seq_len-1:].max(), predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/pileaute/rH/CNN_LSTM_scatterplot.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/rH/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
