...
df=pd.read_csv('data/pileaute/beluchting/KLa.csv', index_col=0)
...
for fold_im_features in all_img_features:      
    ...
    for folder in all_image_folders:
        ...
        if images_in_folder_count > 0:
            ...
            target_per_date[folder]=df['kLa (1/h)'].loc[folder].item() # Save label per date
    ...
    train_indices_folders=np.arange(0, 50)
    val_indices_folders = np.arange(50, 63) 
    ...
    scheduler =  torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, factor=0.85)
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 63)
    ...
    if best_val_loss < 20: #only use this one for final predictions is validation loss is ok
    ...
    plt.plot(df['kLa (1/h)'][all_dates].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
    plt.plot(predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)], '.-', label='predictions (train)', color='orange')
...
plt.ylabel("kLa (1/h)")
plt.legend()
plt.savefig('/home/loesv/all_results/pileaute/KLa/CNN_LSTM.png', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/KLa/CNN_LSTM.pdf', dpi=250)
plt.show()
...
y_true_train = df['kLa (1/h)'][all_dates].iloc[seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test =df['kLa (1/h)'][all_dates].iloc[seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):]
...
# 1:1 line
lims = [
    min(df['kLa (1/h)'][all_dates].iloc[seq_len-1:].min(), predictions.min()),
        max(df['kLa (1/h)'][all_dates].iloc[seq_len-1:].max(), predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/pileaute/KLa/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pileaute/KLa/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
