...
def parse_args():
    ...
    parser.add_argument('--labelpath', type=str, default= 'data/bath/alpha_interpolated.csv', help='Path to load the labels')
    parser.add_argument('--output_path', type=str, default= 'output/bath/alpha_withDNAinput', help='Path for saving models and metrics')    
    ...
    parser.add_argument('--target', type=str, default='alpha', help='Target to train on')
...
if __name__ == "__main__":
    ...
    train_dataset = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-11-23', end_folder='2021-12-01', label_path=args.labelpath, extrainput_path=args.extrainputpath, transform=train_transform) 
    train_dataset_2 = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-12-14', end_folder='2021-12-28', label_path=args.labelpath, extrainput_path=args.extrainputpath,transform=train_transform) 
 
    val_dataset = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-11-23', end_folder='2021-12-01', label_path=args.labelpath, extrainput_path=args.extrainputpath, transform=val_transform)   
    val_dataset_2 = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-12-14', end_folder='2021-12-28', label_path=args.labelpath, extrainput_path=args.extrainputpath, transform=val_transform)   
    ...
    for fold, (train_index, val_index) in enumerate(kfold.split(idx)):
        ...
        if exists:
            ...
            new_head = nn.Sequential(
                nn.Linear(combined_features_size, 16),  
                nn.ReLU(),
                nn.Linear(16, 1)  
                )
            new_head.to(device)
