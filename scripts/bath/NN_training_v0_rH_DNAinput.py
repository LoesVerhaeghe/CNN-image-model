...
def parse_args():
    ...
    parser.add_argument('--extrainputpath', type=str, default= 'data/bath/DNA_interpolated_standardized.csv', help='Path to load the labels')
    parser.add_argument('--labelpath', type=str, default= 'data/bath/v0_rh_interpolated.csv', help='Path to load the labels')
    parser.add_argument('--output_path', type=str, default= 'output/bath/v0_rh_withDNAinput', help='Path for saving models and metrics')    
    ...
    parser.add_argument('--target', type=str, default='v0_rh', help='Target to train on')
...
class CombinedModel(nn.Module):
    ...
    def forward(self, image, lab_data):
        ...
        # if epoch == 0 and i == 0:
        #     print("img_features:", img_features.shape, img_features.mean().item(), img_features.std().item())
        #     print("extrainputs:", extrainputs.shape, extrainputs.mean().item(), extrainputs.std().item(), extrainputs.dtype)
...
if __name__ == "__main__":
    ...
    train_dataset = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-11-19', end_folder='2021-12-15', label_path=args.labelpath, extrainput_path=args.extrainputpath, transform=train_transform) 
    train_dataset_2 = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-12-21', end_folder='2021-12-28', label_path=args.labelpath, extrainput_path=args.extrainputpath,transform=train_transform) 
 
    val_dataset = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-11-19', end_folder='2021-12-15', label_path=args.labelpath, extrainput_path=args.extrainputpath, transform=val_transform)   
    val_dataset_2 = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, start_folder='2021-12-21', end_folder='2021-12-28', label_path=args.labelpath, extrainput_path=args.extrainputpath, transform=val_transform)   
    ...
    for fold, (train_index, val_index) in enumerate(kfold.split(idx)):
        ...
        if exists:
            ...
            del model
            model = []        
            full_dataset = dataset_extrainputs.MicroscopicImagesBath(root=args.imagedatapath, 
            ...
            with torch.no_grad():
                for ii, data in enumerate(full_loader, 0):
                    if args.target == 'bulking':
                        ...
                        preds = best_model(inputs,extrainputs)
        ...
        #               " || ", net_time, 'sec')        
        print('fold',fold, ", ep:", converged_on+1, "|" ,
            f"train loss: {train_losses[converged_on]}, "
                # f"train acc: {round(train_accs[converged_on] * 100, 2):.2f}%", 
                    f"val loss: {val_losses[converged_on]}, "
                    # f"val acc: {round(val_accs[converged_on] * 100, 2):.2f}%",
                    " || ", net_time, 'sec')     
