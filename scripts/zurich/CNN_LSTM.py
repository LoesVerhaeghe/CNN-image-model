...
output_path='output/zurich/TSS/TSS_LR5e5'
...
for fold_im_features in all_img_features:      
    ...
    for folder in all_image_folders:
        ...
        for subfolder in listdir(path_to_folder):
            ...
            if subfolder: #results are much better if sludge recycle images are also included..
    ...
    #_, _ = train_model(model_temp, train_loader, val_loader, optimizer, None, criterion, epochs=300)
    ...
    scheduler =  torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=5, factor=0.85)
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 260)
...
plt.figure(figsize=(14, 3), dpi=200)
plt.rcParams.update({'font.size': 12})    
plt.plot(df_TSS['SST_TSS'][all_dates].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.ylabel("TSS (mg/L)")
plt.legend()
plt.savefig('/home/loesv/all_results/zurich/TSS_eff/CNN_LSTM.png', dpi=250)
plt.savefig('/home/loesv/all_results/zurich/TSS_eff/CNN_LSTM.pdf', dpi=250)
plt.show()
...
y_true_train = df_TSS['SST_TSS'][all_dates].iloc[seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = TSS_predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test = df_TSS['SST_TSS'][all_dates].iloc[seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):]
...
# 1:1 line
lims = [
    min(df_TSS['SST_TSS'][all_dates].iloc[seq_len-1:].min(), TSS_predictions.min()),
        max(df_TSS['SST_TSS'][all_dates].iloc[seq_len-1:].max(), TSS_predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/zurich/TSS_eff/CNN_LSTM_scatterplot.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/zurich/TSS_eff/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
