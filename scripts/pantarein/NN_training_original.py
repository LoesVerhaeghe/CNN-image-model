...
def parse_args():
    ...
    parser.add_argument('--labelpath', type=str, default= 'data/pantarein/SVI.csv', help='Path to load the labels')
    parser.add_argument('--output_path', type=str, default= 'output/pantarein/output_SVI_original_withtest', help='Path for saving models and metrics')    
...
##################################################################
from src import dataset_pantarein
...
if __name__ == "__main__":
    ...
    train_dataset = dataset_pantarein.MicroscopicImagesPantareinOriginal(root=args.imagedatapath,start_folder='2021-01-04', end_folder='2022-04-20', label_path=args.labelpath, transform=train_transform) 
    val_dataset = dataset_pantarein.MicroscopicImagesPantareinOriginal(root=args.imagedatapath, start_folder='2021-01-04', end_folder='2022-04-20', label_path=args.labelpath, transform=val_transform)   
        ...
        if exists:
            ...
            if args.tl:                
                ...
                if 'resnet' in args.backbone:
                    ...
                    model = timm.create_model(args.backbone, pretrained=True, 
                                              num_classes=n_classes, head_init_scale=0.001, #NO CLASSIFICATION HEAD!!!!!! we will define it ourselves
                                              drop_path_rate=drop_path_rate)
                else:
                    model = timm.create_model(args.backbone, pretrained=True, 
                                              num_classes=n_classes)
            ...
            del model
            model = []        
            full_dataset = dataset_pantarein.MicroscopicImagesPantareinOriginal(root=args.imagedatapath, 
