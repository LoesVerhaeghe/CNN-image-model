...
output_path='output/pantarein/svi/SVI_LR1e4'
images_base_folder = 'data/pantarein/image_data_pantarein_structured'
df=pd.read_csv('data/pantarein/SVI.csv', index_col=0)
...
for fold_im_features in all_img_features:      
    ...
    for folder in all_image_folders:
        ...
        if images_in_folder_count > 0:
            target_per_date[folder]=df['SVI'].loc[folder].item() # Save label per date
    ...
    train_indices_folders=np.arange(0, 66)
    val_indices_folders = np.arange(66, 76) 
    test_indices_folders=np.arange(76, 91)
    ...
    optimizer = optim.Adam(model_temp.parameters(), lr=1e-2)
    ...
    criterion = nn.MSELoss()
    train_val_indices_folders=np.arange(0, 76)
    ...
    plt.plot(df['SVI'][all_dates].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
...
plt.figure(figsize=(14, 3), dpi=200)
plt.rcParams.update({'font.size': 12})    
plt.plot(df['SVI'][all_dates].iloc[seq_len-1:91], '.-', label='Measurements', color='blue')
plt.plot(predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)], '.-', label='Model predictions (train)', color='orange')
plt.plot(predictions.iloc[len(train_indices_folders)+len(val_indices_folders):91], '.-', label='Model predictions (test)', color='red')
plt.xlabel("Time")
plt.ylabel("SVI")
plt.legend()
plt.savefig('/home/loesv/all_results/pantarein/SVI/CNN_LSTM.png', dpi=250)
plt.savefig('/home/loesv/all_results/pantarein/SVI/CNN_LSTM.pdf', dpi=250)
plt.show()
...
y_true_train = df['SVI'][all_dates].iloc[seq_len-1:].iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_pred_train = predictions.iloc[0:len(train_indices_folders)+len(val_indices_folders)]
y_true_test = df['SVI'][all_dates].iloc[seq_len-1:].iloc[len(train_indices_folders)+len(val_indices_folders):91]
y_pred_test = predictions.iloc[len(train_indices_folders)+len(val_indices_folders):91]
...
# 1:1 line
lims = [
    min(df['SVI'][all_dates].iloc[seq_len-1:].min(), predictions.min()),
        max(df['SVI'][all_dates].iloc[seq_len-1:].max(), predictions.max())
        ]
...
plt.tight_layout()
plt.savefig('/home/loesv/all_results/pantarein/SVI/CNN_LSTM_scatterplot.png',  bbox_inches='tight', dpi=250)
plt.savefig('/home/loesv/all_results/pantarein/SVI/CNN_LSTM_scatterplot.pdf',  bbox_inches='tight', dpi=250)
plt.show()
